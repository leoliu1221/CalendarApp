...
RCMenu.prototype = {
    ...
    getDefaultURL: function() {
		var defaultID = this.getDefaultId();
		return this.getURL(defaultID);
    },
    
	
    getDefaultSeqNum: function() {
		var defaultID = this.getDefaultId();
		return this.services[defaultID].seqNum;
	},
    ...
    getDefaultId: function() {
		...
		var smallestSeqNum = this.services[0].seqNum;
		for (i=0; i < this.services.length; i++) {        
			if (this.services[i].seqNum <= smallestSeqNum) {
				defaultID = this.services[i].id;
				smallestSeqNum = this.services[i].seqNum;
            }
		}
		return defaultID;
    },
    ...
    getHeight : function(id) {
        ...
        // return "75%, *";  // TODO: default height is 75% target content, 25% related content
		return "0";  		 // let the iframe determines the default height
    },
    ...
    getUseEdit : function(id) {
        ...
        return "0";       // default is no display options  
    },
		
	setValue : function(id,newVal) {
        for (i=0; i < this.services.length; i++) {
            if (this.services[i].id == id) {
                this.services[i].value = newVal;
                break;
                }
        }
    }
