...
init:function(oWin,oDoc,oForm,url_up,url_dn,url_close,nDelayTime){
    ...
    this.undeEnd="</span>";
    this.selectSpanStart="<span style='width:100%;display:block' class='spanNormalElement' onmouseover='ptTAObj_win1.SetHighColor(this)' ";
...
AddHandler:function(objText){
     objText.onblur=function(){
        ptTAObj_win1.bLostFocus = true;
        if(this.obj.useTimeout) { 
            if (!browserInfoObj2.isSafari || !ptTAObj_win1.bInitialOnBlur)
                ptTAObj_win1.StartTimeout();
            else if (ptTAObj_win1.bInitialOnBlur)
                ptTAObj_win1.bInitialOnBlur = false;
        }
     }
},

KeyUpDown:function(e) {
    if (typeof ptTAObj_win1.oWin == 'undefined') 
        return;
    this.bTabPressed = false;
    this.bKeyUp = false;
    this.bKeyDown = false;
    var intKey=-1;
    if (this.theTextBox)
        addchg_win1(this.theTextBox);
    if (ptTAObj_win1.oWin.event) 
        intKey=ptTAObj_win1.oWin.event.keyCode;
    else
        intKey = e.which;
    ...
    if(intKey==this.UPARROW){
        this.bKeyUp = true;
        ptTAObj_win1.MoveHighlight(-1);
        ptTAObj_win1.GrabHighlighted();
        return true;
    }
        ...
        ptTAObj_win1.HideTheBox();         //ICE#1850813000
        return true;
    }
...
GiveOptions:function(e) {
    ...
    if (ptTAObj_win1.oWin.event) {    
        intKey=ptTAObj_win1.oWin.event.keyCode;
        this.theTextBox=ptTAObj_win1.oWin.event.srcElement;
    }
    else{
        intKey = e.which;
    ...
    if (ptTAObj_win1.CancelTypeAhead(this.theTextBox)) return false;

    
    if (intKey == this.BACKSPACE || this.lastIntKey != this.BACKSPACE) {
        ptTAObj_win1.EraseDelayTimeout();
    }
    ...
    ptTAObj_win1.resetTextBox(this.theTextBox);
    if(this.theTextBox.value.length==0&&!this.isOpera){
        ptTAObj_win1.arrOptions=new Array();
        ptTAObj_win1.HideTheBox();
        ptTAObj_win1.strLastValue='';
    }
    ...
    if (this.theTextBox.value.length==0) 
        ...
        if ( ptTAObj_win1.bInProcess || (this.objLastActive!=this.theTextBox) ||
             ((this.theTextBox.value.length!=0) && (this.theTextBox.value.length > ptTAObj_win1.strLastValue.length)) ||
             (this.theTextBox.value.indexOf(ptTAObj_win1.strLastValue)!=0) ||
             (ptTAObj_win1.arrOptions.length==0 && !this.bNoResults) ||
             (this.theTextBox.value.length <= ptTAObj_win1.strLastValue.length) ) {
              ptTAObj_win1.StartDelayTimeout();
        } else {
            if (this.theTextBox.obj.nMaxRows==ptTAObj_win1.arrOptions.length)
                ptTAObj_win1.DoTypeAhead();
            else {
                ptTAObj_win1.BuildList(this.theTextBox.value);
            }
        }
    }
    return true;
...
DoTypeAhead:function(){
    if ((ptTAObj_win1.arrOptions.length > 0 && !this.bNoResults)
        ...
        if (!this.bNoResults) {
            ptTAObj_win1.strLastValue=this.theTextBox.value;
            return;
        }
        }

    if (ptTAObj_win1.theTextBox.value.length==0 || typeof ptTAObj_win1.theTextBox.value == "undefined")      //791953
        ptTAObj_win1.HideTheBox();
    else {
        ptTAObj_win1.bMadeRequest=true;
        ptTAObj_win1.bInProcess = true;
        ptTAObj_win1.TypeAhead(ptTAObj_win1.theTextBox.value);
        ptTAObj_win1.objLastActive=ptTAObj_win1.theTextBox;
        ptTAObj_win1.bInProcess = false;
    }
    ptTAObj_win1.strLastValue=this.theTextBox.value;
},
...
buildChoices:function(){
    ...
    if (this.RestoreTextBox())
        {    
        this.BuildList(ptTAObj_win1.strLastValue.replace(/\\/g,'\\\\'));
        bMadeReqiest = false;
        this.startPos = 0;
        this.bFoundSpecialKeycode = false;
        }
...
TAFieldHasFocus:function() {
    ...
    if (typeof PFieldList_win1!='undefined' && PFieldList_win1) {
         for (var i = 0; i < PFieldList_win1.length; i++) {
            var fname = PFieldList_win1[i][0];    
            var nStop = PFieldList_win1[i][1];
            for (var j=0; j<nStop; j++) {        
                ...
                if (nStop>1) {    
                    nsuffix = '$'+j;   
                    name = fname+nsuffix;
                } else if (ptTAObj_win1.oDoc.getElementById(fname+'$'+j)) {
                    nsuffix = '$'+j;   
                    name = fname+nsuffix;
                }
                var el = ptTAObj_win1.oDoc.getElementById(name);
    ...
    if (typeof EFieldList_win1!='undefined' && EFieldList_win1) {
         for (var i = 0; i < EFieldList_win1.length; i++) {
            var fname = EFieldList_win1[i][0];    
            var nStop = EFieldList_win1[i][1];
            for (var j=0; j<nStop; j++) {        
                var name = fname;    
                var nsuffix = "";
                if (nStop>1) {    
...
SetMaxRange:function(theTextBoxInt,nMatchLength) {
    ...
    var divgblrId = null;
    this.maxRange=3;
    var XYposition = ptCommonObj2.getPosition2(theElement, "win1");
...
BuildList:function(theText){
    ...
    var theMatches = null;
    var matchGrid = null;
    var xPosElement = ptTAObj_win1.oDoc.getElementById("pt_typeahead");

    this.SetMatchesArr(theText);
    var mLengthA= ptTAObj_win1.arrMatches.length;
    ...
    if (matchGrid) {
           ...
           ptTAObj_win1.oDoc.getElementById("OptionsList_0").className="spanNormalElement";
           ...
           if (this.ndisplay == 0) {    // display typeahead box above the typeahead item
              ...
              if (this.bStartNewList) {
                ...
                if(this.theTextBox.obj.showNoMatchMessage) 
                   ptTAObj_win1.oDoc.getElementById("pt_typeahead").innerHTML="<span class='noMatchData' style='white-space:nowrap;background-color: rgb(226,226,226);'>"+this.noMatchingDataMessage+"</span>";    
                else 
                   this.HideTheBox();
        }
...
SetElementPosition:function(theTextBoxInt, selectedPosX, selectedPosY ){
    ...
    var el = ptTAObj_win1.oDoc.getElementById("pt_typeahead0");
    if (el && typeof el != 'undefined')
    ...
    var xPosElement = ptTAObj_win1.oDoc.getElementById("pt_typeahead");
        ...
        if(theTextBoxInt.obj.useTimeout){
            xPosElement.onmouseout=ptTAObj_win1.StartTimeout;
            xPosElement.onmouseover=ptTAObj_win1.EraseTimeout;
        }
        else {
    ...
    //commented out until feed back
        // ptTAObj_win1.AutoWinAdj(xPosElement);   
},
SetOpValue:function(theTextBox) {
    ...
    ptTAObj_win1.opValue=elOP.value;
    elOP.value=1;
    return;
},
RestoreOpValue:function(theTextBox) {
    ...
    var elOP=this.oWin.document.getElementById(name+"$op");
    if (elOP==null) return;
    if (ptTAObj_win1.opValue != -1) {
        elOP.value = ptTAObj_win1.opValue;
        ptTAObj_win1.opValue = -1;
    }
    return;
},
...
SetMatchesArr:function(xCompareStr){
    this.countForId=0;
    this.currentTotalRow=0;        //789561
    ptTAObj_win1.arrMatches=new Array();
    ...
    if (!this.bFoundSpecialKeycode) {
        ...
        if(pattern.length > 0) {        //791953 
            ...
            for(var i=0;i<ptTAObj_win1.arrOptions.length;i++){
                var theMatch=ptTAObj_win1.arrOptions[i][this.theTextBox.obj.colnum].match(regExp);
                if (theMatch) {
                        tmpArray[ptTAObj_win1.arrMatches.length]=ptTAObj_win1.arrOptions[i][this.theTextBox.obj.colnum];
                        ptTAObj_win1.arrMatches[ptTAObj_win1.arrMatches.length]=ptTAObj_win1.arrOptions[i];
                }
            }
        }
    } 
        ...
        for(var i=0;i<ptTAObj_win1.arrOptions.length;i++) {
            var str = ptTAObj_win1.arrOptions[i][this.theTextBox.obj.colnum];
...
MakeMatchesArr:function(xCompareStr){
    ...
    for(var i=this.startPos;(i<ptTAObj_win1.arrMatches.length && (i-this.startPos)<this.maxRange);i++){
        var matchRowArr=new Array();
        for (var j=0;j<ptTAObj_win1.arrOptions[i].length;j++) {
            if (j==this.theTextBox.obj.colnum) {
                matchRowArr[matchRowArr.length]=this.CreateBold(ptTAObj_win1.arrMatches[i][j],xCompareStr,i);
            }
            else        //789561 - selection on other sugguestion columns
                matchRowArr[matchRowArr.length]=this.CreateBoldOtherCol(ptTAObj_win1.arrMatches[i][j],i);
        }
...
CheckDup:function(tmpArray,value){
    /*if (typeof(bSearchDialog_win1)=="undefined" || !bSearchDialog_win1)
        return false;
...
ScrollUp:function(){
    ...
    if (this.startPos < 0 )
        this.startPos = 0;
    this.BuildList(ptTAObj_win1.strLastValue);
    var xPosElement = ptTAObj_win1.oDoc.getElementById("pt_typeahead");
...
ScrollDown:function(){
    this.startPos = this.startPos + this.maxRange-1;
    if (this.startPos < ptTAObj_win1.arrMatches.length)
    this.BuildList(ptTAObj_win1.strLastValue);
    var xPosElement = ptTAObj_win1.oDoc.getElementById("pt_typeahead");
    xPosElement.onmouseover();
    this.MoveHighlight(1);
...
CreateGrid:function(matchArray,mLengthA) {
    ...
    if (browserInfoObj2.isiPad && browserInfoObj2.isSafari) {
        var sCloseTAResult = "<img align='right' src='"+this.url_close+"'/ onclick='ptTAObj_win1.HideTheBox()'>";
        matchGrid=matchGrid+this.trStart;
        matchGrid=matchGrid+sCloseTAResult; 
        matchGrid=matchGrid+this.trEnd;
    }
    if (browserInfoObj2.isiPad && browserInfoObj2.isSafari)  { 
        var sScrollUp = "<img align='absmiddle' src='"+this.url_up+"'/ onclick='ptTAObj_win1.ScrollUp()'>";
    } else {
        var sScrollUp = "<img align='absmiddle' src='"+this.url_up+"'/ onmouseover='ptTAObj_win1.ScrollUp()'>";
    }    
    ...
    if (((this.startPos+this.maxRange) < ptTAObj_win1.arrMatches.length) && mLength>=this.maxRange){
          if (browserInfoObj2.isiPad && browserInfoObj2.isSafari) { 
            var sScrollDown = "<img align='absmiddle' src='"+this.url_dn+"'/ onclick='ptTAObj_win1.ScrollDown()'>";
        } else {
            var sScrollDown = "<img align='absmiddle' src='"+this.url_dn+"'/ onmouseover='ptTAObj_win1.ScrollDown()'>";
        }
...
CreateBold:function(xStr,xTextMatch,xVal) {
    selectSpanMid="onclick='ptTAObj_win1.SetText("+xVal+")'"+"id='OptionsList_"+this.countForId+"' theArrayNumber='"+xVal+"'>";
    if (!this.bFoundSpecialKeycode) {
...
MoveHighlight:function(xDir){
    if(this.currentValueSelected>=-1){
        ...
        if (newValue == this.currentTotalRow && ((this.startPos+this.maxRange) < ptTAObj_win1.arrMatches.length)){
            this.ScrollDown();
...
SetHighColor:function(theTextBox){
    ...
    var odd=false;
    var totalCol=ptTAObj_win1.arrMatches[0].length;
    for(i=0;i<this.currentTotalRow;i++){
        var tObj = ptTAObj_win1.oDoc.getElementById('OptionsList_'+i*totalCol);
    ...
    if (theTextBox) {
        var selectOBJ=ptTAObj_win1.oDoc.getElementById('OptionsList_'+this.currentValueSelected);
        this.currentValueSelected = selectOBJ.getAttribute("theArrayNumber") - this.startPos;        //point to the row
    }
    else
        var selectOBJ=ptTAObj_win1.oDoc.getElementById('OptionsList_'+this.currentValueSelected*totalCol);
...
IsSelectedUnique:function(xVal){
    ...
    selectedValue = this.DecodeValue(ptTAObj_win1.arrMatches[xVal][this.theTextBox.obj.colnum]);
    if (xVal > 0) 
       aboveValue = this.DecodeValue(ptTAObj_win1.arrMatches[xVal -1][this.theTextBox.obj.colnum]);

    if (xVal + 1 < ptTAObj_win1.arrMatches.length) 
       belowValue= this.DecodeValue(ptTAObj_win1.arrMatches[xVal + 1][this.theTextBox.obj.colnum]);
...
GetIndexBasedOnSelectedVal:function(xVal) {
   ...
   var i = xVal;
   var aboveValue;
   var selectedValue = this.DecodeValue(ptTAObj_win1.arrMatches[xVal][this.theTextBox.obj.colnum]);

   while (i > 0) {
       aboveValue = this.DecodeValue(ptTAObj_win1.arrMatches[i -1][this.theTextBox.obj.colnum]);
...
UpdateControlField:function(xVal) {
    ...
    document.win1.ICAction.value = val;
    aAction_win1(document.win1, val);
},
SetText:function(xVal){
    tryFocus(this.theTextBox);
    this.theTextBox.value=this.DecodeValue(ptTAObj_win1.arrMatches[xVal][this.theTextBox.obj.colnum]);
    
    if (!ptTAObj_win1.IsSelectedUnique(xVal)) {
...
AutoWinAdj:function(xPosElement){
    ptTAObj_win1.bodyScrollLeft=ptTAObj_win1.oDoc.body.scrollLeft; 
    ptTAObj_win1.bodyScrollTop=ptTAObj_win1.oDoc.body.scrollTop;
    var t1=parseInt(xPosElement.offsetHeight,10);
    var t2=parseInt(document.body.offsetHeight,10)-(parseInt(xPosElement.style.top,10)-ptTAObj_win1.oDoc.body.scrollTop);
    if (t1>t2)
        ptTAObj_win1.oDoc.body.scrollTop=t1-t2+10;

    t1=parseInt(xPosElement.offsetWidth,10);
    t2=parseInt(document.body.offsetWidth,10)-(parseInt(xPosElement.style.left,10)-ptTAObj_win1.oDoc.body.scrollLeft);
    if (t1>t2) {
        if (ptTAObj_win1.oDoc.win1.ICActionPrompt.value=='true') {
             var o=ptTAObj_win1.oWin.parent.document.getElementById('popupContainer');
             ptTAObj_win1.promptWidth = Math.max(0,o.style.width.replace('px',''));
             var newW=ptTAObj_win1.promptWidth+t1-t2+20+'px';
             o.style.width = newW;
             o=ptTAObj_win1.oWin.parent.document.getElementById('popupFrame');
             o.style.width = newW;
             o=ptTAObj_win1.oWin.parent.document.getElementById("popupTitleBar");
             o.style.width = newW;
         }
         var selectedPosX=0;
         var selectedPosY=0;
         ...
         if ((t1-t2+20) > selectedPosX)
             ptTAObj_win1.oDoc.body.scrollLeft=selectedPosX-20;
         else
             ptTAObj_win1.oDoc.body.scrollLeft=t1-t2+20;
            }
},
AutoWinUnAdj:function(){
    ptTAObj_win1.oDoc.body.scrollLeft=ptTAObj_win1.bodyScrollLeft; 
    ptTAObj_win1.oDoc.body.scrollTop=ptTAObj_win1.bodyScrollTop;        
    if (!ptTAObj_win1.oDoc.win1.ICActionPrompt || ptTAObj_win1.oDoc.win1.ICActionPrompt.value!='true') {
        ptTAObj_win1.promptWidth=0;return;
    }
    if (ptTAObj_win1.promptWidth==0) return;
    var o=ptTAObj_win1.oWin.parent.document.getElementById('popupContainer');
    o.style.width = ptTAObj_win1.promptWidth+'px';
    o=ptTAObj_win1.oWin.parent.document.getElementById('popupFrame');
    o.style.width = ptTAObj_win1.promptWidth+'px';
    o=ptTAObj_win1.oWin.parent.document.getElementById("popupTitleBar");
    o.style.width = ptTAObj_win1.promptWidth+'px'; 
...
ClearResultData:function() {
    ptTAObj_win1.arrHeaders.length = 0;
    ptTAObj_win1.arrOptions.length = 0;
    ptTAObj_win1.arrMatches.length = 0;
},
DecodeArrOptions:function() {
    if (typeof ptTAObj_win1.arrOptions == 'undefined') 
        return;
    for (var i=0;i<ptTAObj_win1.arrOptions.length;i++) {
        for (var j=0;j<ptTAObj_win1.arrOptions[i].length;j++) {
            ptTAObj_win1.arrOptions[i][j] = this.DecodeValue(ptTAObj_win1.arrOptions[i][j]);
        }
    }
},
HideTheBox:function(){
    //commented out until feed back
    //ptTAObj_win1.AutoWinUnAdj();
    this.bGrabHighlighted = false;
    var el=ptTAObj_win1.oDoc.getElementById('pt_typeahead0');
    if (el && typeof el != 'undefined')
        el.style.display='none';
    ptTAObj_win1.oDoc.getElementById('pt_typeahead').style.display='none';
    ptTAObj_win1.currentValueSelected=-2;
    ptTAObj_win1.EraseTimeout();
    ptTAObj_win1.EraseDelayTime();
    this.startPos = 0;
    
    ptTAObj_win1.ClearResultData();    
},
...
EraseDelayTime:function(){
    clearTimeout(ptTAObj_win1.isDelayTiming);
    ptTAObj_win1.isDelayTiming=false;
},
StartDelayTimeout:function(){
    if (!ptTAObj_win1.theTextBox) return;
    ptTAObj_win1.isDelayTiming=setTimeout("ptTAObj_win1.DoTypeAhead()",this.nDelayTime);
},
...
EraseTimeout:function(){
    clearTimeout(ptTAObj_win1.isTiming);
    ptTAObj_win1.isTiming=false;
},
StartTimeout:function(){
    if (!ptTAObj_win1.theTextBox) 
        return
    ptTAObj_win1.isTiming=setTimeout("ptTAObj_win1.HideTheBox()",ptTAObj_win1.theTextBox.obj.theVisibleTime);
}
}

var ptTAObj_win1 = new PT_typeahead();