...
function setSaveText_win1(txt)
{
var saveobj = document.getElementById("SAVED_win1");
if (saveobj)
   {
   document.getElementById("ptStatusText_win1").innerHTML = txt;
   showhide(saveobj, true);
   document.getElementById("saveWait_win1").style.display = "";
   }
}

function processing_win1(opt,waittime)
{
var waitobj = document.getElementById("WAIT_win1");
var saveobj = document.getElementById("SAVED_win1");
var saveWaitObj = document.getElementById("saveWait_win1");
if (opt == 0)
{
...
{
    showhide(waitobj, false);
    setSaveText_win1('Saved');
    if (saveWaitObj)
        saveWaitObj.style.display = "none";
    bTimer = setTimeout("processing_win1(0)",waittime);
}
}
...
function doKeyDown_win1(evt)
{
var target = getEventTarget(evt);

if (target && target.form && target.form.name != "win1")
    return findHandler("doKeyDown_" + target.form.name, evt);

nLastKey_win1 = getKeyCode(evt);
if (nLastKey_win1 != "\t".charCodeAt(0)) {

if (isGridNav(evt))
    ptGridObj_win1.doArrowNavGrid(evt);

if (isTypeAheadEl(evt))
     ptTAObj_win1.KeyUpDown(evt);

    return true;
}

if (isCtrlKey(evt))
    return tryFocus(oFirstTab_win1);
if (isAltKey(evt) || (isShiftKey(evt) && !isGridNav(evt)))
    return true;

if (target == oLastTab_win1)
{
if (oFirstTB_win1)
    {
    if (!bTabOverTB_win1)
...
if (oFirstPg_win1)
    {
    if (!bTabOverPg_win1)
        return tryFocus(oFirstPg_win1);
    }
if (bTabOverNonPS_win1 || oFirstTB_win1 != null || oFirstPg_win1 != null)
    return tryFocus(oFirstTab_win1);
return true;
}
if (target == oLastTB_win1)
{
  if (oFirstPg_win1)
  {
    if (!bTabOverPg_win1)
  ...
  if (bTabOverNonPS_win1 || oFirstPg_win1 != null)
    return tryFocus(oFirstTab_win1);
  return true;
}

if (target == oLastPg_win1)
{
  if (bTabOverNonPS_win1)
    return tryFocus(oFirstTab_win1);
  return true;
}

if (nLastKey_win1 == "\t".charCodeAt(0) && isTypeAheadEl(evt))
{
    ptTAObj_win1.KeyUpDown(evt);
    return true;
}
...
function getLastKey_win1()
{
var nTemp = nLastKey_win1;
nLastKey_win1 = 0;
return nTemp;
}

function doKeyUp_win1(evt)
{
...
if (keyChar == "\r" || key == 27)
    ...
    if (target != null && target.id.indexOf("$delete") != -1)
        {
        if(!evt)return;
        DeleteCheck2_win1(target.id);
        return;
        }
    if (key == 27  && typeof (bDoModal_win1) != "undefined" && bDoModal_win1 && modalID != null) 
        {
        doUpdateParent(document.win1, '#ICCancel');
        return;
        }   
    return;
    }
...
if (bAlt)
    {
    if (bCtrl || altKey_win1.indexOf(keyChar) < 0)
    ...
    if (ctrlKey_win1.indexOf(keyChar) < 0)
        bNotMine = true;
    }
else
    {
    if (baseKey_win1.indexOf(keyChar) < 0)
    bNotMine = true;
    }
...
if (target && target.name)
    document.win1.ICFocus.value = target.name;

if (code == "A8")
    if (!doDeleteKey_win1(target))
        return false;
if (code == "A\xbf")
{
    if (window.FindString_win1
    && document.win1.ICFind
    && findScroll(target))
    {
        if (!FindString_win1(document.win1.ICFind))
            return false;
    }
else
    return false
...
if (saveWarningKeys_win1.indexOf(code) >= 0)
{
   var url = "javascript:submitAction_win1(document.win1, \'#KEY" + code;
   if (code == 'A5')
         url = url  + "\', " + target.id + ")";
   else
        url = url + "\')";
...
if (key == 13 && !ptCommonObj2.isSearchPage(target.form)) //enter key pressed
{
 if (modalZoomName != null) {
    doUpdateParent(document.win1, '#ICReturn');
    return;
 }
 else if (typeof event != "undefined")
    submitAction_win1(document.win1, event.srcElement.id);
 else // for firefox
    submitAction_win1(document.win1, evt.target.id);
}else
{
if (target && target.form)
    addchg_win1(target);
if (code == 'A5')
 submitAction_win1(document.win1, "#KEY" + code + operation, target.id);
else {
 if (bAlt && (key == 49)) //Alt+1 = Saving
    setSaveText_win1('Saving...', true, false);
 submitAction_win1(document.win1, "#KEY" + code);
 }
}
...
{
return keyHandler_win1(keyCode, bIsAlt, bIsCtrl);
}

function keyHandler_win1(keyCode, bIsAlt, bIsCtrl)
{
...
{
    code = "A";
    if (bIsCtrl || altKey_win1.indexOf(keyChar) < 0)
        return false;
}
else if (bIsCtrl)
{
...
function doDeleteKey_win1(obj)
{
if (!window.DeleteCheck2_win1)
    return false;
...
if (document.getElementById(buttonid))
    return DeleteCheck2_win1(buttonid);
...
function doInsertKey_win1(obj)
{
if (!window.AddMultiple_win1)
    return false;
var scroll = findScroll(obj);
if (!scroll)
    return false;
...
if (bHasInsertBtn)
{
    if (scroll.id.match(/\$scrollm\$/))
        return AddMultiple_win1(document.win1.ICAddCount);
...
if (window.doKeyPress_win1 == null)
{
window.doKeyPress_win1 = function (evt)
    {
    var target = getEventTarget(evt);
    if (target && target.form)
         addchg_win1(target);
    ...
    if (key==27)    
        ...
        if (typeof(MOpopupObj_win1.bIs_shown) != "undefined" && MOpopupObj_win1.bIs_shown)
            return MOpopupObj_win1.StopAccess(evt);   // close mouseover popup
         if (typeof (bDoModal_win1) != "undefined" && bDoModal_win1)
            return true;
   ...
   /*var rc = ShowOrCloseModalOnEnter(key, target, "#KEY" + code);
    ...
    if ((key == 13) && isTypeAheadEl(evt) && ptTAObj_win1.IsGrabHighlighted())
        {
    ptTAObj_win1.HideTheBox();
    return false;
        }
...
var oFirstTab_win1 = null;
var oLastTab_win1 = null;
var oFirstTB_win1 = null;
var oLastTB_win1 = null;
var oFirstPg_win1 = null;
var oLastPg_win1 = null;
...
if (obj.tabIndex && obj.tabIndex >= 0)
{
    ...
    {
        if (oLastTab_win1 == null || obj.tabIndex > oLastTab_win1.tabIndex)
            oLastTab_win1 = obj;
        if (oFirstTab_win1 == null || obj.tabIndex < oFirstTab_win1.tabIndex)
            oFirstTab_win1 = obj;
    }
    else if (obj.tabIndex < nFirstPgIndex)
    {
        if (oLastTB_win1 == null || obj.tabIndex > oLastTB_win1.tabIndex)
            oLastTB_win1 = obj;
        if (oFirstTB_win1 == null || obj.tabIndex < oFirstTB_win1.tabIndex)
            oFirstTB_win1 = obj;
    }
    else
    {
        if (oLastPg_win1 == null || obj.tabIndex > oLastPg_win1.tabIndex)
            oLastPg_win1 = obj;
        if (oFirstPg_win1 == null || obj.tabIndex < oFirstPg_win1.tabIndex)
            oFirstPg_win1 = obj;
    }
}
}

function setEventHandlers_win1(sFirst, sLast, bMac)
{
var focus1, focus2;
focus1 = function (evt) {doFocus_win1(this, true, true);};
focus2 = function (evt) {doFocus_win1(this, false, true);};
fchange = function(evt) {addchg_win1(this); };
...
if (window.setKeyEventHandler_win1 == null)
{
    window.setKeyEventHandler_win1 = function ()
    {
    if (typeof(baseKey_win1) != "undefined")
        {
        document.onkeyup = doKeyUp_win1;
        if (baseKey_win1.indexOf("\r") >= 0 || baseKey_win1.indexOf("\x1b") >= 0)
            document.onkeypress = doKeyPress_win1;
        }
    document.onkeydown = doKeyDown_win1;
    }
}
var oTop = null;
var oBottom = null;
...
function setFocus_win1(fldname, indx, oDoc0)
{
...
if (bRichTextEnabled == 0)
   ...
   if (oDoc.win1)
      {
      obj = oDoc.win1.elements[fldname];
      if (!obj)
          obj = oDoc.win1[fldname];
      }
...
function tryFocus0New(obj)
{
  if (obj && typeof obj.focus != "undefined" && !obj.disabled && obj.style.visibility!="hidden")
   ...
   if (window.focusHook)
    ...
    if (gn != null)
       ptGridObj_win1.doScrollOnFocus(gn, obj);
    return false;
    }
return true;
}
...
function setFocusSecPageMessage_win1()
{
var obj;
...
function positionWAIT_win1(){
var waitobj = null;
var savedobj = null;
var objFrame =  top.frames['TargetContent'];
if (objFrame) {
  waitobj = objFrame.document.getElementById("WAIT_win1");
  savedobj = objFrame.document.getElementById("SAVED_win1");
}
else {
  waitobj = document.getElementById("WAIT_win1");
  savedobj = document.getElementById("SAVED_win1");
}
...
if (typeof ptGridObj_win1 != "undefined" && ptGridObj_win1 && gn != null)
{
...
function initTypeAheadEl(el) {
        ...
        var serverScript = "ptTAObj_win1.oWin.pAction_win1(ptTAObj_win1.oDoc.win1,'" + icAction + "');";
        el.obj = oWin.ptTAObj_win1.SetProperties(el, 0, form.ICTypeAheadID, serverScript, true, false, false, true, true, 500); //default search 'Begin with'
 }
 ...
 function isTypeAheadEvtTgt(el) {
        ...
        if (typeof el.obj != 'undefined')
            return true;
        else if (typeof PFieldList_win1 != 'undefined' && PFieldList_win1) {
            var fname = el.name.split('$')[0];
            for (var i = 0; i < PFieldList_win1.length; i++) {
                if (fname == PFieldList_win1[i][0] || el.name == PFieldList_win1[i][0]) {
                    initTypeAheadEl(el);
                    return true;
                }
            }
        ...
        if (ptCommonObj2.isSearchSearchPage(el.form)) {
            if (typeof EFieldList_win1 != 'undefined' && EFieldList_win1) {
                for (var i = 0; i < EFieldList_win1.length; i++) {
                    if (fname == EFieldList_win1[i][0] || el.name == EFieldList_win1[i][0]) {
                        initTypeAheadEl(el);
                        return true;
                    }
                }
...
function preSubmitWorkCenter_win1(arg1, arg2) {
...
function preSubmitProcessSpellcheck_win1(form, name) {
...
function preSubmitProcessCKEDITOR_win1(form, name) {
    if (typeof CKEDITOR != 'undefined') UpdateEditorLinkedField();
}

function preSubmitProcess_win1(form, name) {
 
    if (name.indexOf('#ICQryDownload') == -1) { processing_win1(1, 3000); }
    if ((name.indexOf('$delete') > 0 || name.indexOf('$srt') > 0) && typeof (preID_win1) != 'undefined' && preID_win1 != null) preID_win1 = null; //for grid row highlight
    if (form == null) {
        form = document.forms['win1'];
        form.style.display = 'block';
    }
    preSubmitProcessSpellcheck_win1(form, name);
    preSubmitProcessCKEDITOR_win1(form, name);
}

function aAction0_win1(form, name, params, bAjax, bPrompt)
{
if (typeof bSearchDialog_win1 != 'undefined' && !bDoModal_win1 && bSearchDialog_win1 && form.ICAction.value.indexOf("\x1b") >=0) 
    {
...
if (typeof bAjax != "undefined" && !bAjax && (typeof params == "undefined" || params == null || params == "") && (typeof popupObj_win1.isModalWidget == "undefined" || !popupObj_win1.isModalWidget))
    form.submit();
else if (typeof params == "undefined" || !params)
loader=new net2.ContentLoader(postUrl_win1,form,name,null,null,null,null,null,bAjax, bPrompt);
else
 loader=new net2.ContentLoader(postUrl_win1,form,name,null,null,null,params,null,bAjax, bPrompt);
}

function pAction_win1(form, name, efieldname)
{
if(typeof CKEDITOR != 'undefined')UpdateEditorLinkedField();
...
if (typeof form.ICTypeAheadID != 'undefined' && form.ICTypeAheadID.value != '') {
    ...
    var ret = aAction_win1(form, name, "", true, true);
    form.ICActionPrompt.value = flag;
    return ret;
}
else {
    form.ICActionPrompt.value = 'true';
    return aAction_win1(form, name, "", true, true);
}
}

function mAction_win1(form, name, height, width, title, bModalElement, bSizeOnLoad)
{
    form.ICAction.value = name;
	form.ICXPos.value = ptCommonObj2.getScrollX();
    form.ICYPos.value = ptCommonObj2.getScrollY();
    aAction_win1(form, name, null, true, false);
}
...
function doModlessOnLoad_win1() {
if (window.name.length == 0)
    window.name = "win1";
document.win1.target = window.name;
...
function setModWinParentPIA() {
    if (setModWinParent())
        document.win1.ICResubmit.value = 0;
}

function doCancelMsg() {
    aAction_win1(document.win1, '#ICCancel');
}
...
function doCancel(sCancelName) {
    ...
    if (typeof window.bMFormSubmit != 'undefined') {
        if (!window.bMFormSubmit) 
            document.win1.ICAction.value = "None";
        else {
    ...
    if (browserInfoObj2.isIE && typeof window.modWin != 'undefined' && window.modWin != null) {
        ...
        var fm = win.document.win1;
        if (!fm) fm = window.document.win1;
        fm.ICAction.value = sCloseId;
        document.win1.ICAction.value = "None";
    }
    try {
        document.win1.target = winParent.name;
        if (document.win1.ICAction.value != "None") return;
        if (typeof window.bMFormSubmit != 'undefined')
            winParent.modWinClosed = null;
        winParent.nResubmit = 0;
    
        if (window.document.win1.enctype.indexOf('multipart') != -1 && typeof window.bMFormSubmit != 'undefined' && !window.bMFormSubmit) {
            winParent.document.win1.ICAction.value = "None";
            winParent.MTop().ptCommonObj2.hidePopupMask();
            closeModal(modalID);
        }
        /*else if (!document.getElementById(sCloseId) && typeof bSearchDialog_win1 != 'undefined' && bSearchDialog_win1)        
            winParent.aAction_win1(document.win1, '#ICModClosed'); *///close all children
        else if (sCloseId != "#ICCancel")
            aAction_win1(document.win1, sCloseId);
        else
            winParent.aAction_win1(document.win1, sCloseId);
...
function closeModWin_win1() {
    ...
    if (window.modWin) {
        try {
        window.modWin.document.win1.ICAction.value = '#ICCancel';
...
function onParentUnload_win1() {
    if ((typeof window.modWin != 'undefined') && window.modWin)    closeModWin_win1();
}

function doModalOnLoad_win1(bStartUp) {
    ...
    if (bStartUp) {
        ...
        if ((mTop.modlessId == -1 || mTop.modlessId != mTop.modId) 
                && typeof winParent.document.win1 != 'undefined')
            winParent.processing_win1(0, 3000);
    ...
    if (window.document.win1.enctype.indexOf('multipart') != -1)
        window.bMFormSubmit = false;
}

function doUpdateParent(form, name) {
    if (winParent.bDefer && name.indexOf("#ICCancel") != -1 && bPromptPage_win1) {
        winParent.bCleanHtml = false;
        winParent.loader = null;
        winParent.document.win1.ICStateNum.value = document.win1.ICStateNum.value;
        winParent.aAction_win1(winParent.document.win1, '#ICCancel');
    } else {
        form.target = winParent.name;
        form.ICAction.value = name;
        if (typeof CKEDITOR != 'undefined') UpdateEditorLinkedField();
        winParent.loader = null;
        winParent.aAction_win1(form, name);
    }
}

function doModalMFormSubmit_win1(form, name) {
    ...
    submitAction_win1(form, name);
    doModalMFormClose_win1();
}

function doModalMFormClose_win1() {
        winParent.bUpLoadProgress  = true;
        winParent.nResubmit = 1;
        winParent.document.win1.ICStateNum.value = document.win1.ICStateNum.value;
        winParent.loader = null;
        winParent.aAction_win1(winParent.document.win1, 'Resubmit');
...
function doUpdateFirstParent(form, name, firstParent) {
        ...
        firstParent.aAction_win1(form, name);
        if (typeof window.winParent != 'undefined' && window.winParent.winParent != null)
            ptCommonObj2.hidePopupMask(window.winParent);
        else
            ptCommonObj2.hidePopupMask(MTop());
...
function resizeZoomGrid(id, w, h) {
    if (typeof gridList_win1 == 'undefined') return;
    var nMinHeight = 140;  // default minimum height for g zoom
    var exp = "win1" + "div";  //assuming that the overall g content id is always win1divgName$index
    var gNameParts = id.split(exp);
    var gToolBar = "win1" + "divG" + gNameParts[1];
    ...
    for (var g = 0; g < gridList_win1.length; g++) {        
        if (gridList_win1[g][0] == gNameParts[1]) {            
            var gFirstRowID = "tr" + gNameParts[1] + "_row" + (gridList_win1[g][1] + 1); //id of the first data row, needed for resizing
            break;
        }
    }
...
getRelatedActions = function(srchUrl, fldId)
{
	...
	// submitAction_win1(document.win1, fldId);
	...
	if (bRemoteRA) {
		...
		document.win1.ICAction.value = fldId;
		...
		var sLoader = new net2.ContentLoader(actionurl, document.win1, fldId, null,
						function() {
...
var bIsPagelet_win1 = "false", sPageletName_win1 = "";
ResetGlyph_win1 = function() {
	if (typeof(sGlyphList_win1) !== "undefined" && sGlyphList_win1 !== "") {
		HideGlyph(sGlyphList_win1);
	}
}

setPageletInfoInCtxmenu_win1 = function(isPagelet,sPgltName) {
	bIsPagelet_win1 = (isPagelet === "false") ? "false" : "true";
	sPageletName_win1 = (typeof(sPgltName) !== "undefined" && sPgltName && sPgltName !== "") ? sPgltName : "";
}
