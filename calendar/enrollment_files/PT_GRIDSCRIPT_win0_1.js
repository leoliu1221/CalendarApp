...
getScrollPos:function(gridID)
{
    ...
    if (typeof ptGridObj_win0.gridArr=="undefined") 
        return {x:0, y:0};   
...
initGrid: function(gid) {
    if (typeof gid == "undefined" || !gid || typeof gridList_win0 == "undefined") return;
        ...
        for (var j = 0; j < gfields.length; j++)
        {
            ...
            {     
                ...
                if (key == 37 && j > 0) // left
                {
                  ...
                  ptGridObj_win0.doScrollLeft(gn,ptCommonObj2.getLeftPos(obj2));                 
                  }
                  return;
                }
...
if (ptGridResizeObj_win0.lMin==0)
    ptGridResizeObj_win0.lMin=ptCommonObj2.getWidth(oghl_div);
if (ogh_div && typeof ogh_div != 'undefined' && ogh_div.id && (ogh_div.id.substring(0,6) == 'divghr'))    
    ptGridResizeObj_win0.offset = ptCommonObj2.getWidth(oghl_div);
...
tst.display='block';
ptGridResizeObj_win0.clientX = evt.clientX;
...
TDselMove : function(evt)
{
 ...
 ptGridResizeObj_win0.TDcellWidth = ptGridResizeObj_win0.TDcellWidth + (evt.clientX - ptGridResizeObj_win0.clientX);
 ptGridResizeObj_win0.clientX = evt.clientX;
...
{  
  ...
  {  
    ...
    if (ogbl_div && typeof ogbl_div != 'undefined')
    {
        ...
        ogbl_div.style.width=Math.max(ptCommonObj2.getWidth(ogbl_div)+rOffset,ptGridResizeObj_win0.lMin);
...
PT_gridDD.prototype = {
    ...
    {
        var refToThis = this;
        var obj = document.getElementById('win0divPAGECONTAINER');
        if (!obj) return;
    ...
    __initDragDropElement : function(e)  // onmousedown event
    {
        ...
        if(parentEl){
            ...
            if(obj){
                ptGridDDObj_win0.drag_minX = ptCommonObj2.getLeftPos(obj);
                ptGridDDObj_win0.drag_minY = ptCommonObj2.getTopPos(obj);
        ...
        if(ptGridDDObj_win0.srcArr[ptGridDDObj_win0.idDragged][1]){
            ptGridDDObj_win0.CloneArr[ptGridDDObj_win0.idDragged].style.top = ptCommonObj2.getTopPos(ptGridDDObj_win0.srcArr[ptGridDDObj_win0.idDragged][0]) + 'px';
            ptGridDDObj_win0.CloneArr[ptGridDDObj_win0.idDragged].style.left = ptCommonObj2.getLeftPos(ptGridDDObj_win0.srcArr[ptGridDDObj_win0.idDragged][0]) + 'px';
        }
    ...
    __slideElementBackIntoItsOriginalPosition : function(numId)
    {
        ...
        var targetX = ptCommonObj2.getLeftPos(ptGridDDObj_win0.srcArr[numId][0]);
        var targetY = ptCommonObj2.getTopPos(ptGridDDObj_win0.srcArr[numId][0]);;
