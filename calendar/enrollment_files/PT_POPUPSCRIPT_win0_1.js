...
StartPopup:function(evt,hoverId,popupId, nWidth, nHeight, labeltext, helptext)   // called by an html control's onclick event to display popup
{
  ...
  else
  {
    ...
    if (HelppopTitle) {        
        ptEvent.add(HelppopTitle,"mousedown",HelppopupObj_win0.InitMovePopup);       
...
InitMovePopup:function(e){
    ...
    HelppopupObj_win0.startPosWindow = [XYposition.x,XYposition.y];  
    if (HelpDragframe)  {
...
MovePopup:function(e) {
    ...
    var HelpDragframe = document.getElementById('HelpDragFrame');   // frame to be dragged
    if(HelppopupObj_win0.moveCounter>=1 && HelpDragframe){        
        var mx = e.clientX;
        var my = e.clientY; 

            HelpDragframe.style.left = HelppopupObj_win0.startPosWindow[0] + mx - HelppopupObj_win0.startEventPos[0] + 'px';       
        var mx = e.clientX;
        var my = e.clientY;
            HelpDragframe.style.top = HelppopupObj_win0.startPosWindow[1] + my - HelppopupObj_win0.startEventPos[1] + 'px';         
    }
},
